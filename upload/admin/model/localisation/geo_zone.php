...
class ModelLocalisationGeoZone extends Model
{
    public function addGeoZone($data)
    {
        ...
        if (isset($data['zone_to_geo_zone'])) {
            for ($i = 0; $i < count($data['zone_to_geo_zone']); $i++) {
                foreach ($data['zone_to_geo_zone'][$i]['zone_id'] as $value) {
                    $this->db->query("DELETE FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$geo_zone_id . "' AND country_id = '" . (int)$data['zone_to_geo_zone'][$i]['country_id'] . "' AND zone_id = '" . (int)$value . "'");

                    $this->db->query("INSERT INTO " . DB_PREFIX . "zone_to_geo_zone SET country_id = '" . (int)$data['zone_to_geo_zone'][$i]['country_id'] . "', zone_id = '" . (int)$value . "', geo_zone_id = '" . (int)$geo_zone_id . "', date_added = NOW()");
                }
            }
        }
